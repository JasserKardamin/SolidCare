...
class dmd
{
    ...
    public string $infos;
    public string $date;
    public function read_DMD_nurse($cin, $pdo)
    {
        try {
    ...
    public function read_DMD_patient($cin, $pdo)
    {
        try {
            $query = $pdo->prepare("SELECT * FROM DMDS WHERE CIN_patient  = $cin AND status != 'refused' AND status != 'accepted'");
            $query->execute();
            return $query->fetchAll();
        } catch (PDOException $e) {
            die('PDO Error: ' . $e->getMessage());
    ...
    public function read_DMD_patient_wr($cin,$pdo)
    {
    ...
    public function create_DMD($CIN_nurse, $CIN_patient, $infos, $date, $pdo)
    {
        try {
    ...
    public function update_DMD($id, $status, $pdo)
    {
    ...
    public function delete_DMD($id, $pdo)
    {
        try {
            $query = $pdo->prepare('
